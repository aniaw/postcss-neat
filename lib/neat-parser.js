...
var atRules = {
  ...
  'omega': function omega(query, direction, media) {
    return _grid2.default.omega(query, direction, options, media);
  },
  'outer-container': function outerContainer(maxWidth, media) {
    return _grid2.default.outerContainer(maxWidth, options, media);
  },
  'pad': function pad(media) {
    for (var _len = arguments.length, padding = Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
      padding[_key - 1] = arguments[_key];
    }

    return _grid2.default.pad(padding, options, media);
  },
  'row': function row(display, media) {
    return _grid2.default.row(display, options, media);
  },
  'shift': function shift(columns, containerColumns, media, direction) {
    return _grid2.default.shift(columns, containerColumns, direction, options, media);
  },
  'show-grid': function showGrid(columns, containerColumns, media, location, direction) {
    return _grid2.default.showGrid(columns, containerColumns, location, direction, options, media);
  },
  'span-columns': function spanColumns(columns, containerColumns, display, media, direction) {
    return _grid2.default.spanColumns(columns, containerColumns, display, direction, options, media);
  }
};
